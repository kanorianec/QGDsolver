...
namespace qgd
{
    defineTypeNameAndDebug(RSWETau,0);
...
void Foam::qgd::
RSWETau::correct(const QGDThermo& qgdThermo)
{
    if (isA<shallowWaterQGDThermo>(qgdThermo))
    {
        /*const shallowWaterQGDThermo& refThermo = 
            refCast<const shallowWaterQGDThermo>(qgdThermo);
        
        const dimensionedScalar& alpha = refThermo.alpha();
        ...
        this->tauQGD_ = alpha * this->hQGD_ / sqrt(magg * h);
